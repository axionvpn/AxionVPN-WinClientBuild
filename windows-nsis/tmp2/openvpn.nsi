...
!define AXION_PACKAGE_NAME "AxionVPN"
...
; Windows.
InstallDir "$PROGRAMFILES\${AXION_PACKAGE_NAME}"
...
InstallDirRegKey HKLM "SOFTWARE\${AXION_PACKAGE_NAME}" ""

;--------------------------------
;Modern UI Configuration

; Compile-time constants which we'll need during install
;!define MUI_WELCOMEPAGE_TEXT "This wizard will guide you through the installation of ${PACKAGE_NAME} ${SPECIAL_BUILD}, an Open Source VPN package by Axion.$\r$\n$\r$\nNote that the Windows version of ${PACKAGE_NAME} will only run on Windows XP, or higher.$\r$\n$\r$\n$\r$\n"
!define MUI_WELCOMEPAGE_TEXT "This wizard will guide you through the installation of AxionVPN, a front-end for OpenVPN designed to work on Axion's network.$\r$\n$\r$\nThis software  will only run on Windows XP, or higher.$\r$\n$\r$\n$\r$\n"

!define MUI_COMPONENTSPAGE_TEXT_TOP "Select the components to install/upgrade.  Stop any ${AXION_PACKAGE_NAME} processes or the ${PACKAGE_NAME} service if it is running.  All DLLs are installed locally."

;!define MUI_COMPONENTSPAGE_SMALLDESC
;!define MUI_FINISHPAGE_SHOWREADME "$INSTDIR\doc\INSTALL-win32.txt"
!define MUI_FINISHPAGE_RUN_TEXT "Start AxionVPN"
!define MUI_FINISHPAGE_RUN "$INSTDIR\bin\AxionVPN.exe"
...
!ifdef USE_OPENVPN_GUI
	LangString DESC_SecOpenVPNGUI ${LANG_ENGLISH} "Install AxionVPN"
!endif
...
Section -pre
	...
	guiNotRunning:
		...
		RMDir /r "$SMPROGRAMS\AxionVPN"

		; Stop & Remove previous AxionVPN service
		DetailPrint "Removing any previous AxionVPN service..."
...
Section /o "${PACKAGE_NAME} Service" SecService
	...
	${If} ${SectionIsSelected} ${SecAddShortcutsWorkaround}
		...
		CreateDirectory "$SMPROGRAMS\AxionVPN\Shortcuts"
		;CreateShortCut "$SMPROGRAMS\${PACKAGE_NAME}\Shortcuts\${PACKAGE_NAME} Sample Configuration Files.lnk" "$INSTDIR\sample-config" ""
		CreateShortCut "$SMPROGRAMS\${AXION_PACKAGE_NAME}\Shortcuts\AxionVPN log file directory.lnk" "$INSTDIR\log" ""
	...
	!insertmacro WriteRegStringIfUndef HKLM "SOFTWARE\${PACKAGE_NAME}" ""		 "$INSTDIR" 
...
Section /o "${PACKAGE_NAME} GUI" SecOpenVPNGUI
	...
	File "${OPENVPN_ROOT}\bin\AxionVPN.exe"
	#File "${OPENVPN_ROOT}\bin\WinSparkle.dll"


	${If} ${SectionIsSelected} ${SecAddShortcutsWorkaround}
		CreateDirectory "$SMPROGRAMS\AxionVPN"
		CreateShortCut "$SMPROGRAMS\AxionVPN\AxionVPN.lnk" "$INSTDIR\bin\AxionVPN.exe" ""
		CreateShortcut "$DESKTOP\AxionVPN.lnk" "$INSTDIR\bin\AxionVPN.exe"
	${EndIf}
SectionEnd
!endif
...
Section /o "Add Shortcuts to Start Menu" SecAddShortcuts
	...
	CreateShortCut "$SMPROGRAMS\AxionVPN\Uninstall AxionVPN.lnk" "$INSTDIR\Uninstall.exe"
SectionEnd
...
!endif
	...
	${If} "${ARCH}" == "x86_64"
		...
		${If} "$INSTDIR" == "$PROGRAMFILES\AxionVPN"
			StrCpy $INSTDIR "$PROGRAMFILES64\AxionVPN"
		${EndIf}
	${EndIf}
...
Section -post
	...
	WriteRegStr HKLM "SOFTWARE\${AXION_PACKAGE_NAME}" "" "$INSTDIR"

	; Create uninstaller
	WriteUninstaller "$INSTDIR\Uninstall.exe"

	; Show up in Add/Remove programs
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${AXION_PACKAGE_NAME}" "DisplayName" "${AXION_PACKAGE_NAME} ${VERSION_STRING} ${SPECIAL_BUILD}"
	WriteRegExpandStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${AXION_PACKAGE_NAME}" "UninstallString" "$INSTDIR\Uninstall.exe"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${AXION_PACKAGE_NAME}" "DisplayIcon" "$INSTDIR\icon.ico"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${AXION_PACKAGE_NAME}" "DisplayVersion" "${VERSION_STRING}"
...
Section "Uninstall"
	...
	!ifdef USE_TAP_WINDOWS
		ReadRegStr $R0 HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${AXION_PACKAGE_NAME}" "tap"
		${If} $R0 == "installed"
	...
	!ifdef USE_OPENVPN_GUI
		Delete "$INSTDIR\bin\AxionVPN.exe"
		Delete "$DESKTOP\${AXION_PACKAGE_NAME}.lnk"
	!endif
	...
	!insertmacro DelRegKeyIfUnchanged HKCR ".${OPENVPN_CONFIG_EXT}" "${AXION_PACKAGE_NAME}File"
	DeleteRegKey HKCR "${PACKAGE_NAME}File"
	DeleteRegKey HKLM "SOFTWARE\${PACKAGE_NAME}"
	DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${AXION_PACKAGE_NAME}"
